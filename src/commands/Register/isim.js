...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        db.push(`isimler_${member.id}`, ` \` ${name} ${config.registration.symbol} ${age}\` İsim Değiştirme`);
        await guild.members.cache.get(member.id).setNickname(` ${name} ${config.registration.symbol} ${age}`);
        channel.send(embed.setDescription(`Kullanıcının ismi \` ${name} ${config.registration.symbol} ${age}\` olarak değiştirildi!`)).catch(err => console.log(err), client.ytick(message)).then(m => m.delete({timeout: 10000}));
    }
}