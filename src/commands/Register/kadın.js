    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (config.registration.purchase) {
            if (!member.username.includes(config.registration.GuilDTag) && !member.roles.cache.has(config.roles.viprole && config.roles.boosterrole && config.roles.musiciansrole && config.roles.designerrole && config.roles.team)) {
                return channel.send(embed.setDescription(`Bilinmeyen Bir Sıkıntı Oluştu!`))
            }
        }
        await guild.members.cache.get(member.id).setNickname(`${name} ${config.registration.symbol} ${age}`);
        db.add(`kadın_${author.id}`, 1)
        db.add(`toplam_${author.id}`, 1)
              const names = db.get(`isimler_${member.id}`)
        db.push(`isimler_${member.id}`, ` \`${name} ${config.registration.symbol} ${age}\` (<@&${config.registration.woman}>)`);
        db.push(`kke_${member.id}`, `${author} \`${moment(Date.now()).format("LLL")}\` (<@&${config.registration.woman}>)`)
        ...
        if (!names) {
            channel.send(embed.setDescription(`Kullanıcının ismi \`${name} ${config.registration.symbol} ${age}\` olarak değiştirildi ve <@&${config.registration.woman}> rolü verilerek kayıt edildi!`))
        } else {
            channel.send(embed.setDescription(`Kullanıcı başarıyla <@&${config.registration.woman}> olarak kayıt edildi!\n\n Kullanıcının toplamda " ${names.length} " isim kayıtı görüntülendi.\n${names.map((data) => `${data}`).join("\n")}`))
        }
